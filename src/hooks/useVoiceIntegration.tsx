...
} = isVoiceFeatureGated() ? require('./useVoice.js') : {
  useVoice: ({
    enabled: _e
  }: {
...
export function useVoiceIntegration({
  ...
  const voiceInterimTranscript = isVoiceFeatureGated() ?
  // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
  ...
  const interimRange = useMemo((): InterimRange | null => {
    if (!isVoiceFeatureGated()) return null;
