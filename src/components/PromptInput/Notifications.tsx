...
const VoiceIndicator: typeof import('./VoiceIndicator.js').VoiceIndicator = isVoiceFeatureGated() ? require('./VoiceIndicator.js').VoiceIndicator : () => null;
...
function NotificationContent({
  ...
  const voiceError = isVoiceFeatureGated() ?
  // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
  ...
  if (isVoiceFeatureGated() && voiceEnabled && (voiceState === 'recording' || voiceState === 'processing')) {
  ...
  return <>
      ...
      {isVoiceFeatureGated() ? voiceEnabled && voiceError && <Box>
              <Text color="error" wrap="truncate">
                {voiceError}
              </Text>
            </Box> : null}
